...
<template>
  ...
  >
    ...
    >
      ...
      >
        <h2>{{ subTitle }}</h2>
        <div flex="~ col" mt-6px gap-10px>
          <span v-for="text in desc" :key="text">
            {{ text }}
          </span>
        </div>
      </div>
...
.card-container{
  ...
  background-image: linear-gradient(to right,rgba(248,255,255,1),rgba(248,255,255,1)), linear-gradient(to right,#0B8AFA, #47DEF6);
  border-left: 0;
  border-right: 0;
  border-bottom: 0;
  h2{
